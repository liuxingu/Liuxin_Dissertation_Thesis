...
Transition metal dichalcogenides(TMD) semiconductor have the general chemical formula of MX$_2$, where $M$ refers to the transition metal atom($Mo,W$) and $X$ is the chalcogen atom($S$,$Se$,$Te$).
...
There are three different phases in the TMD crystal: hexagonal 2H, rhombohedral 3R, and trigonal 1T' phases. In their most thermal dynamic stable phase 2H polytype, a monolayer consists of a hexagonal plane of transition metal atoms sandwiched between two planes of chalcogen atoms, forming trigonal prismatic coordination.
...
However, this inversion symmetry can be restored in even number multilayer, with one layer is rotated $180^\circ$ relative to the other layer.
...
As a result, strong second-order nonlinear optical processes, such as second-harmonic generation, have been extensively investigated in 3R multilayer\cite{Xu2022-at,Qin2024-xt}.
...
\end{center}
\caption[Crystal structures of TMDC.]{\textbf{a,b,} Side view (top) and top view (bottom) of the 3R and 2H stacking structure. 
...
\end{center}
\caption[Band structures of TMDC.]{\textbf{Band structures of TMDC.} \textbf{a,b,c} Calculated orbital projected band structure for monolayer $MoS_2$, with \textbf{a} contribution from the Mo atom $d$ orbitals;\textbf{b}Total $p$ orbitals dominated by the chalcogen S atom; \textbf{c} Total $s$ orbital.
\textbf{d,e,f,g} Calculated bandstructure of bulk,quadrilayer,bilayer and monolayer TMD(from left to right).
The crossover from indirect to direct bandstructure happens at the monolayer limit.Figure adapted from\cite{Splendiani2010-km,Liu2013-jh}.}
\label{fig.band}
\end{figure}
\end{quote}
\renewcommand{\baselinestretch}{2}
...
Red lines are exciton states obtained from one-photon absorption and blue shaded region is from two-photon absorption corresponding to the p-states.Figures are adapted from\cite{Chernikov2014-vo,He2014-ct}.}
...
\small\normalsize
% --------------
However, the conventional Coulomb potential form fails to reproduce the experimentally observed binding energies and non-hydrogenic exciton spectra in TMDs (Figure~\ref{fig.rydberg_Ex}b).
...
According to this, this electric field induced finite dipole moment can be expressed as $\alpha_{1s} \epsilon$. 
...
The exciton linewidth in directly related to the radiative decay rate, which is proportional to $1 / a_B^2$\cite{Wang2018-my}. Since TMD excitons have very small Bohr radius($\sim 1\,\ nm$), the calculated radiative broadening is on the order of 1 meV \cite{Glazov2015-us,Glazov2014-qz}.
This corresponds to ultrafast intrinsic lifetimes shorter than 1 ps\cite{Moody2015-iz,Robert2018-lt}, which is about two orders of magnitude faster than excitons in GaAs quantum wells\cite{Deveaud1991-yo}. 

Additional broadening occurs from non-radiative decay involving exciton--phonon scattering, intervalley relaxation, and disorder-induced inhomogeneity, which typically increase with temperature\cite{Cadiz2017-ly,Robert2018-if,Ajayi2017-ky,Moody2015-iz}.
Except for homogeneous broadening, inhomogeneous broadening originates from spatial variations in the local environment, including disorder, strain, and dielectric fluctuations, which create a distribution of resonance energies across the sample\cite{Jakubczyk2019-wv}.
...
Polaritons in transition-metal dichalcogenides (TMD) systems are formed when strongly bound excitons interact with photons confined in an optical cavity\cite{Deng2010-ul}. 
...
To realize this, a high quality-factor (Q) microcavity\cite{Vahala2003-yh} is essential, as it minimizes photon losses and prolongs the photon lifetime, thereby strengthening light–matter coupling and enhancing nonlinear polariton interactions\cite{Gu2021-cu,Emmanuele2020-oh,Louca2023-in,Datta2022-po,Lopriore2025-yd,Sigurdsson2024-nm}. 

A Fabry–Pérot cavity with distributed Bragg reflectors (DBRs) provides simple, well-defined modes, making it ideal for polariton studies in 2D materials\cite{Rahimi-Iman2020-tt,Gibbs2011-yv}.
In this structure, each DBR is formed by alternating two dielectric materials with a high refractive index contrast, with each layer thickness set to $l = \frac {\lambda}{4 n_0}$. 
...
Under this design, the DBR acts as a highly reflective mirror within a certain wavelength range, known as the stop band or photonic band\cite{Rahimi-Iman2020-tt}. 
...
$f_0 = c/\lambda_0$ is\cite{Rahimi-Iman2020-tt}:
\begin{equation}
  \frac{\Delta f_0}{f_0}
  \;=\;
...
According to the Fermi’s golden rule, the transition rate $\Gamma_{\omega}$ can be expressed as\cite{Fox2006-hp,Gerry2024-aa}:
\begin{equation}
...
In cavity QED, the vacuum Rabi coupling for a single exciton to the cavity photon can be expressed by the coupling coefficient $g_1$ as\cite{Fox2006-hp,Gerry2024-aa}:
\begin{equation}
...
The effective cavity length $L_{\rm eff}$ consist of the cavity spacer length $L_{\rm eff}$ and the field penetration length $L_{\rm DBR}$, which is given by\cite{Rahimi-Iman2020-tt,Savona1995-vn}:
\begin{equation}
...
And when we looking at the exciton-polairton Hamiltonian, the microscopic coupling $g_{\rm 0}$ appears as the off-diagonal term at fixed in-plane wavevector $k_{\parallel}$\cite{Rahimi-Iman2020-tt,Deng2010-ul}:
\begin{equation}
\begin{aligned}
\hat H_{\mathrm{pol}}
...
The transformation to the lower/upper polariton operators is given by the Hopfield mixing\cite{Rahimi-Iman2020-tt,Deng2010-ul}:
\begin{equation}
\begin{pmatrix}
\hat{p}_{k_{\parallel}} \\
\hat{q}_{k_{\parallel}}
...
\label{eq:Hopfield}
\end{equation}
where $|C_{k_{\parallel}}|^{2}$ and $|X_{k_{\parallel}}|^{2}$ quantify the photonic and excitonic fractions, respectively. It can be defined as\cite{Rahimi-Iman2020-tt,Deng2010-ul}:
\begin{equation}\
\lvert X_{k_\parallel}\rvert^{2}
= \frac{1}{2}\!\left(
1 + \frac{\Delta E(k_\parallel)}{\sqrt{[\Delta E(k_\parallel)]^{2}+4 g_0^{2}}}
\right)
\label{eq:Hopcoefficient_Xk}
\end{equation}
...
1 - \frac{\Delta E(k_\parallel)}{\sqrt{[\Delta E(k_\parallel)]^{2}+4 g_0^{2}}}
\right)
\label{eq:Hopcoefficient_Ck}
\end{equation}
Diagonalizing Eq.~\eqref{eq:H_polariton} yields the lower (LP) and upper (UP) polariton energies\cite{Rahimi-Iman2020-tt,Deng2010-ul}:
\begin{equation}
...
The detuning between the uncoupled modes is defined as\cite{Deng2010-ul}:
\begin{equation}
...
Figure~\ref{fig.Ex_dispersion_detuning} shows polariton dispersion and the corresponded Hopfield coefficient for different detuning situation.
...
\end{center}
\caption[Exciton polariton dispersions and the Hopfield coefficient for lower polariton at different detuning $\Delta$] {\textbf{Exciton polariton dispersions and the Hopfield coefficient for lower polariton at different detunings.} \textbf{a,} $\Delta= 2 g_0$; \textbf{b,} $\Delta\ = 0$; \textbf{c,} $\Delta\ = -2g_0$.
Figures are adapted from\cite{Deng2010-ul}.}
\label{fig.Ex_dispersion_detuning}
\end{figure}
\end{quote}
\renewcommand{\baselinestretch}{2}
\small\normalsize
% --------------
Equivalently, using the 2D oscillator strength per area $f_{2\mathrm{D}}$, we can have the Rabi splitting as\cite{Savona1995-vn}:
\begin{equation}
...
Since the planar (1D) cavity provides an in-plane TE field aligned to the TMD intralayer exciton dipole, the dipole–field overlap $\lvert\mathbf d\!\cdot\!\mathbf E\rvert$ is maximized\cite{Gerry2024-aa,Fox2006-hp}. 
Leveraging the monolayer’s large oscillator strength and the enhancement at the field antinode, the strong coupling regime is easy to access in the TMD embedded microcavity system, even with limited quality factor\cite{Liu2015-hm}. 
The strong coupling is obtained when\cite{Rahimi-Iman2020-tt,Deng2010-ul}
\begin{equation}
