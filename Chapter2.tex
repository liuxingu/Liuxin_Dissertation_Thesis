...
As a result,with increasing layer numbers lifts the $\Gamma$ valence band upward and lowers the $Q$ conduction band, while the $K$ valleys remain nearly unchanged.   % \cite{yong}
...
\label{eq.SOC}
\end{equation}
where $\mathbf{L}$ and $\mathbf{S}$ are the orbital and spin angular momenta, and $\xi(r)$ denotes the radial SOC strength, which increases strongly with atomic number. 
In transition-metal dichalcogenides (TMDs), the heavy transition-metal atoms (Mo, W) provide a strong SOC environment that directly impacts the band edges.
% \cite{Coupled Spin and Valley Physics in Monolayers of  and Other Group-VI Dichalcogenides}
...
Their strong coupling to spin through the $L_z S_z$ interaction results in a large spin splitting, typically several hundred meV, especially for W-based TMD. 
...
$d_{z^2}$ state to nearby orbitals that carry finite angular momentum ($m_l = \pm 1$), most importantly the transition-metal $d_{xz}/d_{yz}$ and chalcogen $p_x/p_y$ states. 
...
In contrast, WX$_2$ compounds exhibit a positive splitting, and the conduction subband ordering is reversed.
The broken inversion symmetry together with strong spin–orbit coupling in monolayer TMDs gives rise to spin–valley coupling, which enables control of valley degrees of freedom through phenomena such as the valley Hall effect and valley-selective optical transitions.
...
which enforces conservation of angular momentum between the electron system and the absorbed or emitted photon. 
The corresponding transition amplitude can be written as:
\begin{equation}
...
$\mathbf{e}_{\pm} =\frac{1}{\sqrt{2}} (\hat{x} \pm i\hat{y})$ are polarization vectors for right- ($\sigma^{+}$) and left- ($\sigma^{-}$) handed circularly polarized light. 
Because $\mathbf{e}_{\pm}$ carries angular momentum projection $\pm \hbar$ along the light propagation axis, a circularly polarized photon transfers angular momentum $\pm 1$ into the electronic system. 
Thus, optical excitation with $\sigma^+$ or $\sigma^-$ light can
be used to probe states that differ in their orbital angular momentum by one unit.

In semiconducting TMD monolayers, the broken inversion symmetry and strong spin--orbit coupling translate these general rules into valley-specific selection rules. 
...
The conduction band minimum is mainly derived from the $d_{z^2}$ orbital ($m_l=0$). 
...
This valley--helicity locking gives rise to valley-selective circular dichroism, as demonstrated in monolayer MoS$_2$ \cite{mak2012}. 
The experimental observation o nearly perfect polarization selectivity provides direct evidence that inversion symmetry
...
symmetry is restored, and the valley-dependent optical selection rules are lost: both
valleys can now couple to both helicities with equal weight. 
As a result, bilayer MoS$_2$ shows no net circular dichroism under optical excitation \cite{mak2012}. 
...
These rules form the microscopic basis of valley-selective optical excitation and detection, establishing light helicity as a powerful handle to manipulate valley degrees of freedom in two-dimensional semiconductors.
...
The Coulomb attraction between the negatively charged electron and the hole can lead to the formation of a bound electorn-hole pair called exciton. 
...
In the presence of free carriers, excitons can interact with the surrounding Fermi sea of electrons or holes, forming new excitation states known as polarons. 
The polaron can be described as an exciton dressed by particle--hole excitations of the Fermi sea, shown as two different branches in the optical spectrum: an attractive polaron at lower energy and a repulsive polaron at higher energy.
...
The relative motion based on the effective mass can be described by the Wannier equation:
\begin{equation}
...
\label{eq:coulomb}
\end{equation}
By solving the Wannier equation with 3D and 2D Laplace operator in polar coordinates, we can obtain the exciton binding energies follow a Rydberg series:
\begin{flalign}
...
In real experiments, TMDs are typically studied in device structures encapsulated by dielectric hBN, which has a relatively low permittivity of about 3. 
This weak screening further enhances the Coulomb interaction, resulting in a much larger exciton binding energy($\sim$ 500 meV) compared to that in GaAs quantum wells($\sim$ tens of meV).
...
Red lines are exciton states obtained from one-photon absorption and blue shaded region is from two-photon absorption corresponding to the p-states.Figures are adpated from[].}
...
In atomic thin layer, the electric field lines generated by an electron–hole pair extend partly within the 2D sheet and partly into the surrounding environment, which have a different permittivity.

By considering the nonlocal dielectric screening, the electron--hole
interaction can be more accurately described by the Rytova--Keldysh potential:
\begin{equation}
...
$r_0$ is the screening length determined by the 2D polarizability and the surrounding dielectric environment(3 to 6 nm in TMD). 
...
Solving Eq.~\eqref{eq:wannier} with $V(r) = V_{\mathrm{eh}}(r)$ yields binding energies of several hundred meV, consistent with experimental observations in monolayer MoS$_2$
and WSe$_2$.

In the presence of free carriers, excitons no longer behave as isolated two-particle quasiparticles. 
Instead, they interact with the surrounding Fermi sea of electrons or
holes, forming new excitation states known as polarons. 
The polaron can be described as an exciton dressed by particle--hole excitations of the Fermi sea, shown as two different branches in the optical spectrum: an attractive polaron at lower energy and a repulsive polaron at higher energy.

\subsection{Exciton in external Electric field}
The motion of the exciton can be separated into two parts: the relative motion of the electron around the hole, which determines the binding energy and size of the exciton, and the center-of-mass (COM) motion of the exciton as a whole across the 2D plane  of the material.
...
And the kinetic energy of the exciton can be defined as:
\begin{equation}
...
In this case, the excitonic Hamiltonian includes an additional term, so that the Wannier equation becomes:
\begin{equation}
\left[-\frac{\hbar^2}{2\mu}\nabla_{\mathbf r}^2 
...
In the weak-field regime, where the applied field is well below the dissociation threshold, the leading correction to the exciton energy is given by the quadratic Stark shift, then then energy shift of the 1s exciton depends on the field can be obtained in terms of:
\begin{equation}
...
The light-matter interaction can be described by the transition probabilities, which can be calculated through the Fermi'Golden rule:
\begin{equation}
...
$\lvert i\rangle$ and $\lvert f\rangle$ are the initial and final states, and $\rho_{\mathrm{ph}}(\omega_n)$ is the photonic density of states at the transition frequency. The matrix element is directly proportional to the oscillator strength $f_0$ as:
\begin{equation}
...
$\mathbf K\!=\!0$ in momentum space. 
Excitons inside the light cone are optically bright and can absorb/emit photons efficiently, whereas excitons outside the cone are dark with respect to direct radiative processes and must first scatter (e.g., by phonons or disorder) back into the lightcone to recombine.

The exciton linewidth in directly related to the radiative decay rate, which is proportional to $1 / a_B^2$. Since TMD excitons have very small Bohr radius($\sim 1\,\ nm$), the calculated radiative broadening is on the order of 1 meV.
This corresponds to ultrafast intrinsic lifetimes shorter than 1 ps, which is about two orders of magnitude faster than excitons in GaAs quantum wells. 
Additional broadening occurs from exciton--phonon scattering, intervalley relaxation, and disorder-induced inhomogeneity, which typically increase with temperature.
Inhomogeneous broadening originates from spatial variations in the local environment, including disorder, strain, and dielectric fluctuations, which create a distribution of resonance energies across the sample.
...
Encapsulation in hBN provides a clean and uniform dielectric environment that suppresses these effects, allowing linewidths to approach the intrinsic homogeneous limit.
...
Polaritons in transition-metal dichalcogenide (TMD) systems arise from the strong coupling between tightly bound excitons and confined cavity photons, forming hybrid light–matter quasiparticles. 
Their large exciton binding energies and strong oscillator strengths enable robust polariton formation, even at elevated temperatures.
The valley-selective optical response of TMD excitons further provides polaritons with spin–valley degrees of freedom.
These features make TMD polaritons a promising platform for studying nonlinear interactions and quantum photonic phenomena.

\subsection{Planar microcavity system}
Polaritons are commonly studied in microcavities, where strong coupling between excitons and confined photons produces hybrid light–matter states. 
...
To realize this, a high quality-factor (Q) microcavity is essential, as it minimizes photon losses and prolongs the photon lifetime, thereby strengthening light–matter coupling and enhancing nonlinear polariton interactions. 
...
If we have the DBR with $N$ pairs of high-($n_2$) and low-($n_1$) refractive index layers, in the normal incidence case, the intensity reflectivity can be approximated by:
\begin{equation}
  R \;=\;
  \left[
...
In the 1D microcavity, the cavity photon exhibits an in-plane energy dispersion as a function of the in-plane wavevector $k_{\parallel}$:
\begin{equation}
