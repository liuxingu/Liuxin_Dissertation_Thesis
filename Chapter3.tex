...
\subsection{2D Stacks Transfer Process}
...
In the PC-assisted dry transfer process, a thin polycarbonate (PC) film supported on a poly-dimethylsiloxane (PDMS) block is mounted on a transparent glass slide and integrated with a micro manipulator stage. 
Under an optical microscope, the PC/PDMS stamp is carefully lowered onto the target flake and brought into contact at a controlled temperature—typically 80–120 $^{\circ}$ for hBN and above 140 $^{\circ}$ for graphite. 
TMD flakes are usually picked up at lower temperatures to minimize defect activation and to reduce bubble and wrinkle formation caused by thermal expansion. 
The flake adheres to the polymer, and by sequentially picking up and releasing individual layers, van der Waals heterostructures are built up.
Figure~\ref{fig.PCtransfer} shows the process flow for the transfer.
...
\begin{figure}
\begin{center}
\includegraphics[width=1\textwidth]{Figures/C3/Fig.PC transfer.jpg} % 
\end{center}
\caption[Schematic of the PC transfer for 2D stacks] {\textbf{Schematic of the PC transfer for 2D stacks}
\textbf{a,} The glass slide with sticked PDMS and PC stamp is dropping down to pick up the top hBN.
\textbf{b,} After heating the stage to above 130 $^{\circ}$, the hBN flake is sticked to PC and can be then picked up during the process of decreasing temperature.
As PC would separated from the substrate when temperature drops.
\textbf{c,d,e,} Process for picking up TMD and graphite layers.
\textbf{f,g,} After picking up all the layers, the stack would be dropped onto a prepared marker chip at temperature between 150-160 $^{\circ}$. 
This temperature range ensures the PC film adheres to the substrate yet releases from the PDMS without harming of the stack on it.
\textbf{h,}After solving the PC encapsulated stack in the Chloroform for 10 mins, we can obtain the van der Waals heterostructures.
\textbf{i,} An optical image with 100 X magnification of the device right after stacking.
}
\label{fig.PCtransfer}
\end{figure}
\end{quote}
...
After the full stack is assembled, it is released onto the final substrate at 140–160 $^{\circ}$. 
...
The TMD (here we use trilayer WSe$_2$ to keep it consistent with Chapter.5 )is modeled as the grounded sheet. 
...
These two parameters can be tuned independently through $V_{tg}$ and $V_{bg}$. The displacement field can be tuned as:
\begin{align}
...
\subsection{Piezoresponse Force Microscopy(PFM)}
...
A schematic of the optical setup we used in the thesis is shown in the Figure~\ref{fig.OS}. 
For excitonic study(Chapter 4,5), we primarily used the collection path shared with the excitation path with a Galvo mirror system. 
...
At the collection path, a 10 X objective focus the beam onto the single mode fiber. The fiber core with a mode field diameter of 4 $\mu m$ serve as confocal pinhole.

For cavity and polariton study(Chapter 6), the Fourier-space imaging is needed to explore the polariton dispersion. 
In contrast to the real-space imaging, Fourier-space imaging first collects lights across the whole sample with different reflected/emitted angle and then focus the beam to the entrance slit of the spectrometer(shown in the left part of the schematic).
