...
In this chapter, I will introduce the fabrication and metrology methods used throughout this thesis. In section

\section{van der Waals heterostructure Fabrication}
...
\end{center}
\caption[FET Schematic] {\textbf{Schematic of the Dual-gate trilayer $WSe_2$ FET.}}
\label{fig.FET}
\end{figure}
\end{quote}
...
The TMD (here we use trilayer $WSe_2$ to keep it consistant with Chapter.5 )is modelled as the grounded sheet. 
...
\end{align}
As a result, we can calculate the total doping density inside the TMD by plug equations \ref{eq:Ctop},\ref{eq:Cbot} into \ref{eq:chargeQ}, Finally, the net carrier doping density can be calculated as
\begin{equation}
...
\label{eq:Efield}
\end{align}
The $\epsilon_{TMD}$ we use for trilayer $WSe_2$ is around 7 in chapter 5.
...
\end{center}
\caption[TMM] {Schematic of the layered structure with propagating wave's electric field and wave vector.The thickness of the dielectric materials is set as L}
\label{fig.TMM}
\end{figure}
\end{quote}
\renewcommand{\baselinestretch}{2}
...
E(x) =
\begin{cases}
E_0^+ e^{i k_0 x} + E_0^- e^{-i k_0 x}, & x < 0, \\[4pt]
E_0^{\prime +} e^{i k_1 x} + E_0^{\prime -} e^{-i k_1 x}, & 0 < x < L, \\[4pt]
E_1^+ e^{i k_2 (x-L)} + E_1^- e^{-i k_2 (x-L)}, & x > L .
\end{cases}
\label{eq:Ex_f}
\end{equation}
