...
\chapter{Enhancing Optical Nonlinearity Through Microcavity System }
...
Compared with bare excitons, polaritons show enhanced nonlinearities due to modified linewidth and collective light--matter interactions. 
The polariton nonlinear shift can be contributed from the exciton-exciton interaction at lower exciton population denstiy\cite{Gu2021-cu,Ciuti2003-zj} and phase-space filling,
...
In this limit, a single polariton can block the transmission of others, giving rise to photon blockade and enabling nonclassical light sources~\cite{Somaschi2016-uk,Verger2006-jz,He2013-od,Delteil2019-pp,Turunen2022-wo}.
...
Embedding 2D materials with 1D microcavity has already been demonstrated~\cite{Paik2024-ip}. 
...
At first, we extract the dielectric function of the trilayer WSe$_2$ A exciton transition based on the Lorentzian model\cite{Fox2010-ly}:  
\begin{equation}
...
Resonant wavelength: 729.4~nm; oscillator strength $f$: 1.4; polaron linewidth: 10~meV; background permittivity: 25.9\cite{Munkhbat2022-ol,Roche2025-jb}.
...
\small\normalsize
% %/////////////////////// 
When we insert the trilayer WSe$_2$ at the antinode of the resonant field in the cavity, the exciton resonant with the cavity mode will strongly interact with optical field, leading to a new excitation quasiparticle--polariton. 
...
where $U_{\mathrm{ex}}$ refers to the exciton–exciton interaction strength\cite{Sanvitto2016-jz,Ciuti2003-zj}. 
...
According to the simulation results, larger excitonic fractions are obtained only for detuning values $\Delta\geq 0$. The maximum degree of anti-bunching is expected when the exciton and cavity photon are close to resonance. 
...
\textbf{c}, Effective interaction strength parameter $\lvert X_{k_{\parallel}} \rvert^4 / \Gamma$ indicating the maximum degree of anti-bunching.
}
\label{fig.6.5}
\end{figure}
\end{quote}
...
Figures~\ref{fig.6.7} shows the device schematic and the simulated reflectance spectrum for D1 and D2.
...
\subsection{k-space Imaging of the Polariton Dispersion}
...
A standard approximation in multiple–quantum-well microcavities gives\cite{Savona1995-vn}:
\begin{equation}
\hbar\Omega \sim \sqrt{\frac{N f}{L_{\mathrm{eff}}}}
\label{eq:Rabi_layers}
\end{equation}
where $f$ is the per-layer oscillator strength of the active transition and $L_{\mathrm{eff}}$ is the optical length of the cavity~\cite{Savona1995-vn}.
...
This square-root scaling is well established in strongly coupled systems such as GaAs and CdTe multiple–quantum-well microcavities\cite{Skolnick1998-pj}, even for multilayer TMD\cite{Zhao2023-ax,Dufferwiel2015-ro},  
where collective enhancement from multiple wells boosts the light–matter interaction and push this to room temperature.
...
This behavior arises from phase-space filling of the repulsive polaron in monolayer WSe$_2$ under doping\cite{Sidler2017-ff}.
...
Experimentally, we indeed observe only a broadening of the dip feature with increasing gate voltage, which can be attributed to linewidth broadening of the polaron species arising from exciton–carrier scattering\cite{Sidler2017-ff,Efimkin2017-kl}.
...
A supercontinuum white laser was used for broadband power dependent reflection measurements, with the wavelength range selected by an acoustic-optic tunable filter (AOTF): 715–755 nm for the trilayer region and 715–750 nm for the monolayer region (see in Figure~\ref{fig.6.10}).
...
These properties establish trilayer WSe$_2$ microcavities as a promising platform for exploring bright, strongly interacting polariton systems with electrical tunability, and for advancing towards the quantum nonlinear regime, where phenomena such as polariton blockade\cite{Delteil2019-pp,Verger2006-jz,Munoz-Matutano2019-cg} and strongly correlated photonic states\cite{Hartmann2006-we, Carusotto2013-mf} are anticipated.
